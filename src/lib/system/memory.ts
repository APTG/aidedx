 ...
 * model weights. Per issue #32, callers should render `—` when this returns
 ...
 * device RAM rather than heap usage — before giving up, and distinguishes
 * "no data at all" from both, so the caller can render an explicit
 * "not supported" label instead of a `—` that looks like a stuck load.
 *
