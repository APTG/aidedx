...
describe("answerStatus", () => {
  ...
  it("computes and renders a plain-text answer for a confident, complete match", async () => {
    ...
    mocks.matchIntent.mockReturnValue({ intent: i, quantitySource: "direct", incomplete: false, unresolved: [] });
  ...
  it("shows a 'couldn't understand' message for a low-confidence match, without calling compute", async () => {
    ...
    mocks.matchIntent.mockReturnValue({ intent: i, quantitySource: "default", incomplete: true, unresolved: [] });
  ...
  it("fills missing slots with defaults and computes when the quantity is recognized but incomplete", async () => {
    ...
    mocks.matchIntent.mockReturnValue({ intent: i, quantitySource: "direct", incomplete: true, unresolved: [] });
    mocks.getService.mockResolvedValue({});
    mocks.computeIntent.mockReturnValue(computeResult());
  ...
  it("keeps the newer answer when a slower, earlier submit() resolves after a faster, later one", async () => {
    ...
    mocks.matchIntent
      .mockReturnValueOnce({ intent: firstIntent, quantitySource: "direct", incomplete: false, unresolved: [] })
      .mockReturnValueOnce({ intent: secondIntent, quantitySource: "direct", incomplete: false, unresolved: [] });
