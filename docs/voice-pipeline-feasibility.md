...
| `nlu-quantity-prepass.ts` (retired, issue #160 §9a)           | synonym pre-pass → 120/120 (§2.6); its synonym list was a fourth untested copy of `src/lib/intent/lang/en.ts`'s `DIRECT_STOPPING` vocabulary — folded into the shared source of truth at `src/lib/intent/lexicon/quantity-en.ts` and the script deleted                             |
